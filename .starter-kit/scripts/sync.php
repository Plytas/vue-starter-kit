...
function cmdStatus(array $flags): void
{
	...
	foreach ($targets as $target) {
		...
		printf(
			"Counts: pending=%d, in-progress=%d, backported=%d, skipped=%d\n",
			$counts['pending'],
			$counts['in-progress'],
			$counts['backported'],
			$counts['skipped']
...
function cmdPrAdd(array $flags, array $rest): void
{
	$key = $flags['key'] ?? null;
	if (!$key) {
		fail(
			"Usage: pr-add --key=<commit:sha|#pr-number> --target=<branch> --status=<pending|in-progress|backported|skipped>\n" .
			"             [--source-ref=<branch>=<id>] [--reason=TEXT] [--confidence=<high|low>]\n" .
			"  --target and --source-ref may be repeated for multiple values.",
			1
		);
	}

	// Strip leading '#' from key to normalise #310 → 310 (commit: keys are stored as-is)
	if (str_starts_with($key, '#')) {
		$key = substr($key, 1);
	}

	// Collect repeatable flags from raw $rest (parseArgs only keeps the last value)
	$targets = collectMultiFlag($rest, 'target');
	$sourceRefs = collectMultiFlag($rest, 'source-ref');

	if (empty($targets)) {
		fail("--target is required. Repeat for multiple: --target=start --target=start-teams", 1);
	}

	$status = $flags['status'] ?? null;
	if (!$status || !in_array($status, ['pending', 'in-progress', 'backported', 'skipped'], true)) {
		fail("--status is required: pending, in-progress, backported, or skipped", 1);
	}
	...
	// Parse --source-ref=<upstream-branch>=<id> entries
	$parsedRefs = [];
	foreach ($sourceRefs as $sr) {
		if (!str_contains($sr, '=')) {
			fail("--source-ref must be in format <branch>=<id>, got: {$sr}", 1);
		}
		[$branch, $refId] = explode('=', $sr, 2);
		assertUpstreamBranch($branch);
		$parsedRefs[$branch] = $refId;
	}

	// Derive sourceRefs from key when not provided explicitly
	if (empty($parsedRefs)) {
		if (str_starts_with($key, 'commit:')) {
			$parsedRefs = ['main' => substr($key, 7)];
		} else {
			$parsedRefs = ['main' => $key];
		}
	}

	$refType = str_starts_with($key, 'commit:') ? 'commit' : 'pr';

	$builtSourceRefs = [];
	foreach ($parsedRefs as $branch => $refId) {
		$builtSourceRefs[$branch] = [
			'type' => $refType,
			'id' => $refId,
			'url' => '',
			'mergedAt' => null,
	...
	if (isset($state['prs'][$key])) {
		fail("Entry {$key} already exists. Use pr-set to update existing entries.", 2);
	}

	// Build per-target state (same object applied to all specified targets)
	$targetState = ['status' => $status];
	if (isset($flags['reason'])) {
		$targetState['reason'] = $flags['reason'];
	}
	$confidence = $flags['confidence'] ?? null;
	if ($confidence !== null && $confidence !== 'null') {
		if (!in_array($confidence, ['high', 'low'], true)) {
			fail("--confidence must be high, low, or null", 1);
		}
		$targetState['confidence'] = $confidence;
	}

	$targetsMap = [];
	foreach ($targets as $t) {
		$targetsMap[$t] = $targetState;
	}

	$entry = [
		'title' => '',
		'url' => '',
		'sourceRefs' => $builtSourceRefs,
		'targets' => $targetsMap,
	];
	...
	echo "  ✓ #{$key} added\n";
	echo json_encode([$key => $entry], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . "\n";
}

function cmdList(array $flags): void
{
	$state = loadState();

	$statusFilter = $flags['status'] ?? null;
	if ($statusFilter !== null && !in_array($statusFilter, ['pending', 'in-progress', 'backported', 'skipped'], true)) {
		fail("Invalid --status: {$statusFilter}. Valid: pending, in-progress, backported, skipped", 1);
	}

	$targetFilter = null;
	if (isset($flags['target']) && $flags['target'] !== 'all') {
		assertTarget($flags['target']);
		$targetFilter = $flags['target'];
	}
	$targets = $targetFilter ? [$targetFilter] : SUPPORTED_TARGETS;
...
/**
 * Collect all values for a repeatable flag from raw argv.
 * parseArgs only keeps the last value when a flag is repeated; this captures all of them.
 *
 * @param array<string> $argv
 * @return array<string>
 */
function collectMultiFlag(array $argv, string $flagName): array
{
	$prefix = "--{$flagName}=";
	$results = [];
	foreach ($argv as $a) {
		if (str_starts_with($a, $prefix)) {
			$results[] = substr($a, strlen($prefix));
		}
	}
...
Commands:
  ...
  list [--target=start|start-teams|all] [--status=pending|in-progress|backported|skipped]
    One line per entry: <key>  <target>  <status>  <reason>.
    Defaults: --target=all, no status filter (shows everything).
  ...
  pr-add --key=<commit:sha|#pr-number> --target=<branch> --status=<status>
         [--source-ref=<branch>=<id>] [--reason=TEXT] [--confidence=<high|low>]
    Create a new ledger entry. --target and --source-ref may be repeated for
    multiple values. Refuses to overwrite an existing entry (use pr-set instead).
    sourceRefs are derived from --key when --source-ref is omitted.
