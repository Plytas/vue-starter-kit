...
return Chisel::script(__DIR__)
	...
	->selected(
		...
		then: function (Chisel $c) use ($paths) {
			...
			// Trait removed via @chisel-2fa text markers below (chisel's
			// AST removeTrait corrupts the following method when unsetting a
			// non-last trait statement); import + interface AST ops are safe.
			$c->php('app/Models/User.php')
	...
	->selected(
		...
		then: function (Chisel $c) use ($paths) {
			...
			// Trait removed via @chisel-passkeys text markers below (chisel's
			// AST removeTrait corrupts the following method when unsetting a
			// non-last trait statement); import + interface AST ops are safe.
			$c->php('app/Models/User.php')
			...
			$c->files(...[
				...$paths['passkey_files'],
				'database/migrations/2026_06_01_000000_replace_spatie_passkeys_with_laravel_passkeys_table.php',
			])->delete();
		},
	)
	->selectedAny(
